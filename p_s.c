...
int		check_ra(t_stack *stack)
{
	...
	if (!(stack->stack_a[tmp_a] > stack->stack_b[tmp_b] && stack->stack_a[0] < stack->stack_b[tmp_b]) && !tmp_a)
		return (9999);
	return (i);
}
...
int		check_rr(t_stack *stack)
{
	...
	if (!(stack->stack_a[tmp_a] > stack->stack_b[tmp_b] && stack->stack_a[tmp_a + 1] < stack->stack_b[tmp_b]) && (!tmp_b || !tmp_a))
		return (9999);
	return (i);
}
...
void	sort3(t_stack *stack)
{
	...
	if (stack->size_b == 3)
	{
		if (stack->stack_b[2] == smallest_num(stack->stack_b, stack->size_b))
			apply_rb(stack);
		if (stack->stack_b[1] == smallest_num(stack->stack_b, stack->size_b))
			apply_rrb(stack);
...
void	sort(t_stack *stack)
{
	...
	if (!is_sorted_a(stack) && stack->size_a == 2)
		apply_sa(stack);
	if (stack->size_a == 3)
		sort_3(stack);
	if (is_sorted_a(stack) && stack->size_b == 0)
		return ;
	sort2(stack);
	middle = stack->size_b / 2;
	while (stack->size_b != 1)
	{
		if (stack->size_b < middle && stack->size_b > 100)
		{
			middle = stack->size_b / 2;
			sort3(stack);
		}
